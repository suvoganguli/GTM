...
% $Id: example5.m 373 2007-07-25 03:22:47Z cox $
...
% Set output to eas,alpha,beta
set_param('gtm_design/SelectOutputs','OutputSignals',...
    'aux.eas,aux.alpha,aux.beta,aux.gamma');

% Simulate
[t,x,y]=sim('gtm_design',[0 10]); 

% Expand output data structure
eval(sprintf('expand(%s);',get_param('gtm_design/NamedStore','VarName')));
...
[ax,h1,h2]=plotyy(t,gamma,t,eas);grid on
xlabel('time (sec)'),
